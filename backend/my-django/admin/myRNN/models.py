...
class MyRNN(object):
    ...
    def kia_predict(self):
        start_date = '2018-1-4'
        end_date = '2021-10-1'
        KIA = data.get_data_yahoo('042660.KS', start_date, end_date)
        #print(KIA.head(3))
        #print(KIA.tail(3))
    ...
    def ram_price(self):
        ...
        train = ram_price[ram_price['data'] < 2000 ] # 2000년 이전
        test = ram_price[ram_price['data'] >= 2000 ] #2000년 이후
        x_train = train['data'][:, np.newaxis]
        y_train = np.log(train['price'])
        tree = DecisionTreeRegressor().fit(x_train, y_train)
        lr = LinearRegression().fit(x_train, y_train)
        x_all = ram_price['data'].values.reshape(-1,1)
